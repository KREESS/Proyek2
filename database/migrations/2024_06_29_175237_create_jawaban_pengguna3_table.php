...
class CreateJawabanPengguna3Table extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('jawaban_pengguna3', function (Blueprint $table) {
            $table->id();
            $table->foreignId('user_id')->constrained()->onDelete('cascade');

            // Kolom untuk menyimpan jawaban dari 20 soal tryout
            for ($i = 1; $i <= 20; $i++) {
                $table->foreignId("soal_tryout_id_$i")->constrained('soal_tryout')->onDelete('cascade')->nullable();
    ...
    {
        Schema::dropIfExists('jawaban_pengguna3');
    }
}
